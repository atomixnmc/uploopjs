    ...
    return html`
      <div style="font-family:sans-serif;max-width:500px;margin:0 auto;padding:1rem;text-align:center;">
        ...
        <p style="color:#888;font-size:0.85rem;">
          Traffic light FSM with constrained transitions, entry hooks, and auto-cycling effect.
        </p>

        <!-- Visual traffic light -->
        <div style="display:flex;flex-direction:column;align-items:center;gap:0.5rem;margin:1.5rem 0;">
          <!-- Light housing -->
          <div style="background:#333;border-radius:16px;padding:1rem;display:flex;flex-direction:column;gap:0.75rem;width:100px;">

            <!-- Red -->
            <div style="width:60px;height:60px;border-radius:50%;margin:0 auto;
              background:${currentState === 'red' ? '#ff4444' : '#551111'};
              box-shadow:${currentState === 'red' ? '0 0 20px #ff4444' : 'none'};">
            </div>

            <!-- Yellow -->
            <div style="width:60px;height:60px;border-radius:50%;margin:0 auto;
              background:${currentState === 'yellow' ? '#ffaa00' : '#554400'};
              box-shadow:${currentState === 'yellow' ? '0 0 20px #ffaa00' : 'none'};">
            </div>

            <!-- Green -->
            <div style="width:60px;height:60px;border-radius:50%;margin:0 auto;
              background:${currentState === 'green' ? '#44cc44' : '#114411'};
              box-shadow:${currentState === 'green' ? '0 0 20px #44cc44' : 'none'};">
            </div>
          </div>

          <div style="font-size:1.5rem;font-weight:bold;color:${color};margin-top:0.5rem;">
            ${label}
          </div>
        </div>
        ...
        <div style="display:flex;gap:0.5rem;justify-content:center;flex-wrap:wrap;margin:1rem 0;">
          ...
          <button @click=${() => send('toggle')}
            style="padding:0.5rem 1rem;background:${state.running ? '#f44336' : '#4caf50'};color:#fff;border:none;border-radius:4px;cursor:pointer;">
            ${state.running ? '⏸ Pause' : '▶ Resume'}
          </button>

          <button @click=${() => send('emergency')}
        ...
        <!-- State visualization -->
        <details style="margin-top:1.5rem;font-size:0.75rem;color:#888;text-align:left;">
          <summary>State Machine Diagram</summary>
          <pre style="background:#f5f5f5;padding:0.75rem;border-radius:4px;overflow-x:auto;">
${light.visualize()}
