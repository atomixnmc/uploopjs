...
async function searchUsersAPI(query, { signal } = {}) {
  ...
  return FAKE_USERS.filter(
    (u) => u.login.includes(q) || u.name.toLowerCase().includes(q),
  );
}
...
const SearchInput = component("SearchInput", {
  view: (state) => html`
    <div style="position:relative;">
      <input
        type="text"
        placeholder="Search users (try 'a', 'bob', 'grace')..."
        .value=${state.query}
        @input=${(e) => searchLoop.send("search", e.target.value)}
...
const UserCard = component("UserCard", {
  view: (state) => html`
    <div
      @click=${() => searchLoop.send("selectUser", state)}
      style="padding:0.75rem;border:1px solid #eee;border-radius:8px;cursor:pointer;
        display:flex;align-items:center;gap:0.75rem;
        background:${state === searchLoop.get().selectedUser
        ? "#f0f0ff"
        : "#fff"};
        transition:background 0.2s;"
    >
      <div
        style="width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#646cff,#ff44aa);
...
export const AsyncDemo = component("AsyncDemo", {
  ...
  view: (state, { send }) => {
          ...
          ${results.length > 0
            ? html`
                <div style="font-size:0.8rem;color:#888;margin-bottom:0.5rem;">
                  ...
                  ${cacheInfo && !cacheInfo.fresh
                    ? html`<span style="color:#ffaa00;">
                        · cached (stale, refreshing...)</span
                      >`
                    : ""}
                  ${cacheInfo && cacheInfo.fresh
    ...
    return html`
      <div
        style="font-family:sans-serif;max-width:550px;margin:0 auto;padding:1rem;"
      >
        <h2>⚡ Async Data Demo</h2>
        <p style="color:#888;font-size:0.85rem;margin-bottom:1rem;">
          Demonstrates <strong>debounce</strong> (300ms),
          <strong>interruptible</strong> fetch, <strong>error</strong> retry
          with backoff, <strong>suspend</strong> loading states, and
          <strong>cache</strong> with TTL + SWR — all declarative, zero
          boilerplate.
        </p>
        ...
        <div
          style="display:flex;gap:0.5rem;margin:0.5rem 0;flex-wrap:wrap;font-size:0.75rem;"
        >
          <span
            style="padding:0.15rem 0.5rem;background:#e8f5e9;border-radius:4px;color:#2e7d32;"
            >🕐 debounce: 300ms</span
          >
          <span
            style="padding:0.15rem 0.5rem;background:#e3f2fd;border-radius:4px;color:#1565c0;"
            >🛑 interruptible</span
          >
          <span
            style="padding:0.15rem 0.5rem;background:#fff3e0;border-radius:4px;color:#e65100;"
            >🔄 error: 3 retries</span
          >
          <span
            style="padding:0.15rem 0.5rem;background:#f3e5f5;border-radius:4px;color:#7b1fa2;"
            >💾 cache: 10s TTL + SWR</span
          >
        </div>
        ...
        <div style="margin-top:1rem;min-height:200px;">
          ${loopState.query
            ? resultsContent
            : html`<div style="text-align:center;padding:2rem;color:#ccc;">
        ...
        <div style="display:flex;gap:0.5rem;margin-top:1rem;flex-wrap:wrap;">
          <button
            @click=${() => searchLoop.send("refresh", loopState.query)}
...
Events: ${searchLoop.events.total} total, ${searchLoop.events
              .rejected} rejected, depth ${searchLoop.events.depth}</pre
          >
        </details>
      </div>
    `;
