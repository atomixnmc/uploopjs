...
function HomePage(state) {
  return html`
    <div style="padding:1rem;">
      ...
      ${_isLoggedIn
        ? html`<p>Logged in as <strong>${_currentUser.name}</strong> (${_currentUser.role})</p>`
  ...
  return html`
    <div style="padding:1rem;">
      ...
      <ul>
        ${USERS.map(u => html`
          <li>
            <a href="/users/${u.id}" @click=${e => { e.preventDefault(); router.navigate(`users/${u.id}`) }}>
              ${u.name} (${u.role})
            </a>
          </li>
        `)}
...
function UserDetailPage(state) {
  ...
  if (!user) return html`<div style="padding:1rem;color:red;">User not found</div>`
  ...
  return html`
    <div style="padding:1rem;">
      ...
      <p>ID: ${user.id}</p>
      <p>Role: ${user.role}</p>
      <a href="/users" @click=${e => { e.preventDefault(); router.navigate('users') }}>← Back to users</a>
    </div>
  `
}
...
function AboutPage() {
  return html`
    <div style="padding:1rem;">
      <h2>📖 About</h2>
      <p>Uploop Router — exact matching, params, guards, layouts, lazy loading.</p>
...
function AdminLayout(state, content) {
  return html`
    <div style="border:2px solid #ffc107;border-radius:8px;padding:0.5rem;">
      <div style="background:#ffc107;color:#000;padding:0.25rem 0.5rem;border-radius:4px 4px 0 0;font-weight:bold;">
        ⚠ Admin Area
      </div>
      ${content}
    </div>
...
function NotFoundPage() {
  return html`<div style="padding:1rem;color:#888;"><h2>404 — Page Not Found</h2></div>`
}
  ...
  view: (state, { send }) => {
    return html`
      <div style="font-family:sans-serif;max-width:700px;margin:0 auto;padding:1rem;">
        ...
        <!-- Nav bar -->
        <nav style="display:flex;gap:0.75rem;padding:0.5rem 0;border-bottom:2px solid #eee;margin-bottom:1rem;flex-wrap:wrap;align-items:center;">
          <a href="/" @click=${router.link('')}
            style="color:${state.path === '' ? '#646cff' : '#333'};text-decoration:none;font-weight:${state.path === '' ? 'bold' : 'normal'};">Home</a>
          <a href="/users" @click=${router.link('users')}
            style="color:${state.path.startsWith('users') ? '#646cff' : '#333'};text-decoration:none;font-weight:${state.path.startsWith('users') ? 'bold' : 'normal'};">Users</a>
          <a href="/about" @click=${router.link('about')}
            style="color:${state.path === 'about' ? '#646cff' : '#333'};text-decoration:none;font-weight:${state.path === 'about' ? 'bold' : 'normal'};">About</a>
          <a href="/admin" @click=${router.link('admin')}
            style="color:${state.path === 'admin' ? '#646cff' : '#333'};text-decoration:none;font-weight:${state.path === 'admin' ? 'bold' : 'normal'};">Admin 🔒</a>

          <span style="flex:1;"></span>

          ${!_isLoggedIn ? html`
            <button @click=${() => send('login')}
              style="padding:0.25rem 0.75rem;background:#4caf50;color:#fff;border:none;border-radius:4px;cursor:pointer;">
              Login as Admin
            </button>
            <button @click=${() => send('loginUser')}
